...
mh_gibbs <- function(y, iter = 2000, thin = 1, burn = iter / 2,
                     ...
                     pst_init = NULL, k_init = NULL, tau_phi = 1e-2,
                     tau_k = 1e-2) {
  ...
  if(is.null(pst_init) && is.null(k_init)) {
    k0 <- k1 <- rep(0, G)
  } else if(is.null(k_init) && !is.null(pst_init)) {
  ...
  for(it in 1:iter) {
    ...
    if("delta0" %in% names(fixed)) {
      ...
      delta0_new_list <- sample_delta("delta0", ptype, y, pst, k0, k1, phi0, phi1, 
                                      delta0, delta1, tau, gamma, tau_k, tau_phi, 
    ...
    if("delta1" %in% names(fixed)) {
      ...
      delta1_new_list <- sample_delta("delta1", ptype, y, pst, k0, k1, phi0, phi1, 
                                      delta0, delta1, 
                                      tau, gamma, tau_k, tau_phi, tau_delta,
                                      alpha, beta, kappa_delta, kappa_t)
    ...
    if('tau' %in% names(fixed)) {
      ...
      mu1 <- t(sapply(pst_new, function(t) mu_cg(k1_new, phi1_new, delta1_new, t)))
